...
struct Polygon
{
    ...
    __device__ __host__ unsigned char GetImageData(unsigned char *image, int sizex, int sizey, float x, float y, int c) const
    {
    ...
    __device__ __host__ float intersect(const Ray & r, float &u, float &v) const
    {
    ...
    __device__ __host__ Point3D getcol(float u, float v) const
    {
        if(!tex)return col;
